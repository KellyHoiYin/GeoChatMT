...
<body ng-controller="MyCtrl as vm">
	...
	<marker ng-repeat="p in vm.positions" position="{{p.pos}}"
		title="pos: {{p.pos}}"></marker> </ng-map>

	<h3>With zoom-to-include-markers='true'</h3>
	<ng-map zoom="11" center="[40.74, -74.18]" scrollwheel="false"
		zoom-to-include-markers="true"> <marker
		ng-repeat="p in vm.positions" position="{{p.pos}}"
		title="pos: {{p.pos}}"></marker> </ng-map>

	<h3>With zoom-to-include-markers='auto'</h3>
	Please click on map to add more markers
	<ng-map zoom="11" center="[40.74, -74.18]" scrollwheel="false"
		zoom-to-include-markers="auto" on-click="vm.addMarker()"> <marker
		ng-repeat="p in vm.positions" position="{{p.pos}}"
		title="pos: {{p.pos}}"></marker> </ng-map>

	<h3>custom-marker zoom-to-include-markers='auto'</h3>
	Please click on map to add more markers
	<ng-map zoom="11" on-click="vm.addMarker()"
		zoom-to-include-markers='auto' center="[40.74, -74.18]"> <custom-marker
		ng-repeat="p in vm.positions" position="{{p.pos[0]}}, {{p.pos[1]}}">
	<div>position: {{p.pos[0]}}, {{p.pos[1]}}</div>
