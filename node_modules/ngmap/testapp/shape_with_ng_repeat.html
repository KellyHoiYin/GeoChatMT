...
<body ng-controller="MyCtrl as vm">
	<ng-map zoom="11" center="[40.74, -74.18]"> <shape
		ng-repeat="shape in vm.allShapes" name="{{shape.name}}"
		stroke-color="#FF0000" stroke-opacity="0.8" path="{{shape.path}}"
		paths="{{shape.paths}}" bounds="{{shape.bounds}}"
		center="{{shape.center}}" radius="{{shape.radius}}" stroke-weight="2"></shape>
	</ng-map>
</body>
</html>
