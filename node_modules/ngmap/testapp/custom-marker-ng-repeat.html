...
<body>
	<div ng-controller="MyCtrl as vm">
		...
		<custom-marker ng-repeat="p in vm.positions track by $index"
			...
			position="{{p.pos}}">
		<div>position {{p.name}}: {{p.pos[0]}}, {{p.pos[1]}}</div>
		</custom-marker> <info-window id="foo" on-mouseover="vm.mouseover()">
		<div ng-non-bindable="">
		...
		<button ng-click="vm.setPositions(vm.positions1)">show
			positions 1</button>
		<button ng-click="vm.setPositions(vm.positions2)">show
			positions 2</button>
		<button ng-click="vm.selectNextCustomMarker()">select next
			marker</button>
		current position: {{vm.currentPosition}}<br /> vm.positions :
		{{vm.positions}}
