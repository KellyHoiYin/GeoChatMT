...
<body ng-app="ngMap" ng-controller="MyCtrl as vm"
	style="text-align: center">
	<h2>Virtual Road Trip with ngMap StreetView</h2>
	<div class="half">
		<label>Origin:</label> <input places-auto-complete
			ng-model="vm.origin" size=40 ng-disabled="vm.drivingMode" /> <br />
		<label>Destination: </label> <input places-auto-complete
			ng-model="vm.destination" ng-disabled="vm.drivingMode" size=40 /> <br />
		<label>Driving Speed :</label> <input ng-model="vm.drivingSpeed"
			type="range" ng-disabled="vm.drivingMode">
		{{vm.drivingSpeed}} km / hour<br />
	</div>
	<div class="half">
		<button ng-click="vm.drive()" style="width: 200px; font-size: 1em">
			{{vm.drivingMode ? 'Stop' : 'Start Driving' }}</button>
		<br /> <input type="radio" ng-model="vm.driverMode" ng-value="true">
		Driver Mode <input type="radio" ng-model="vm.driverMode"
			ng-value="false"> Passenger Mode
	</div>
	<br style="clear: both" />
	<div class="half" style="height: 70%;">
		...
		<directions draggable="true" panel="directions-panel"
			...
			destination="{{vm.destination}}"> </directions> <street-view-panorama
			container="streetview"></street-view-panorama> <marker
			ng-repeat="pos in vm.markers" position="{{[pos.lat(), pos.lng()]}}"></marker>
		</ng-map>
	</div>
	<div class="half" style="height: 70%;">
	...
	<a href="" ng-click="vm.showDirectionsPanel = !vm.showDirectionsPanel">
		{{vm.showDirectionsPanel ? 'Hide' : 'Show'}} Detailed Directions </a>
	<br style="clear: both" />
